...
var $exe = {
  init: function () {
    ...
    if (e.indexOf("exe-epub3") != 0) {
      ...
      if (
        n.indexOf(' class="mediaelement"') != -1 ||
        n.indexOf(" class='mediaelement") != -1
      ) {
    ...
    init: function () {
      ...
      l.each(function (i) {
        ...
        $("dt", e).each(function () {
          ...
          h = $(t).html();
          $(t).html(
            "<a href='#' onclick='$exe.dl.toggle(this);return false' class='exe-dl-" +
              i +
              "-a'><span class='icon'" +
              s +
              ">» </span>" +
        ...
        if (r.length == 1 && i.length == 1) {
          ...
          s.html(
            '<a href="#' +
              ...
              n +
              '" onclick="$exe.hint.toggle(this);return false" style="background-image:url(' +
              $exe.hint.imgs[0] +
              ')">' +
              o +
              "</a>"
      ...
      t.each(function () {
        ...
        if (eP.length) {
          var n =
            ...
            c +
            '"><a href="#" onclick="$exe.iDeviceToggler.toggle(this,\'' +
            eP.attr("id") +
            "','" +
            c +
            '\');return false" title="' +
...
$exe.cloze = {
  ...
  CORRECT: 2,
  change: function (a) {
    var b = $exe.cloze.getIds(a)[0];
    var c = eval(
      document.getElementById("clozeFlag" + b + ".instantMarking").value
    );
    if (c) {
      $exe.cloze.checkAndMarkWord(a);
      var d = document.getElementById("clozeScore" + b);
      d.innerHTML = "";
    }
  },
  submit: function (e) {
  ...
  checkWordAnswer: function (c, d) {
    d = d.trim();
    var e = c.value;
    var f = d;
    var g = $exe.cloze.getIds(c)[0];
    var h = eval(
      document.getElementById("clozeFlag" + g + ".strictMarking").value
    );
    var k = eval(document.getElementById("clozeFlag" + g + ".checkCaps").value);
    if (!k) {
      e = e.toLowerCase();
      f = f.toLowerCase();
    }
    if (e == f) {
      ...
      var m = Math.floor(f.length / 6) + 1;
      var n = 0;
      if (e.length <= m) {
        n = Math.abs(e.length - f.length);
      ...
      for (i = 0; i < 2; i++) {
        ...
        while (o) {
          n = Math.floor(
            (Math.abs(o.length - p.length) + Math.abs(e.length - f.length)) / 2
          );
          var q = Math.min(o.length, p.length);
          for (j = 0; j < q; j++) {
            var a = p.charAt(j);
            var b = o.charAt(j);
            if (a != b) n += 1;
  ...
  onLangChange: function (a) {
    var b = $exe.cloze.getLangIds(a)[0];
    var c = eval(
      document.getElementById("clozelangFlag" + b + ".instantMarking").value
    );
    if (c) {
      $exe.cloze.checkAndMarkLangWord(a);
      var d = document.getElementById("clozelangScore" + b);
      d.innerHTML = "";
    }
  },
  ...
  checkLangWord: function (c) {
    var d = c.value;
    var e = $exe.cloze.getLangAnswer(c);
    var f = e;
    var d = c.value;
    var g = $exe.cloze.getLangIds(c)[0];
    var h = eval(
      document.getElementById("clozelangFlag" + g + ".strictMarking").value
    );
    var k = eval(
      document.getElementById("clozelangFlag" + g + ".checkCaps").value
    );
    if (!k) {
      d = d.toLowerCase();
      f = e.toLowerCase();
    ...
    if (d == f) {
      ...
      for (i = 0; i < 2; i++) {
        ...
        while (o) {
          n = Math.floor(
            (Math.abs(o.length - p.length) + Math.abs(d.length - f.length)) / 2
          );
          var q = Math.min(o.length, p.length);
          for (j = 0; j < q; j++) {
            var a = p.charAt(j);
  ...
  showLangScore: function (a, b) {
    var c = eval(
      document.getElementById("clozelangFlag" + a + ".showScore").value
    );
    if (c) {
      var d = 0;
      var e = document.getElementById("clozelang" + a);
      var f = $exe.cloze.getLangInputs(a);
      for (var i = 0; i < f.length; i++) {
        var g = f[i];
        if (b) {
          var h = $exe.cloze.checkAndMarkLangWord(g);
        } else {
          var h = $exe.cloze.getLangMark(g);
        }
        if (h == 2) {
          d++;
        }
      }
      var j = document.getElementById("clozelangScore" + a);
      j.innerHTML = YOUR_SCORE_IS + d + "/" + f.length + ".";
    }
  },
